...
TYPE T_KPI5_ARRAY IS TABLE OF T_KPI5_REC;
TYPE T_KPI6_ARRAY IS TABLE OF T_KPI6_REC;
...
-- PROCEDURE 1
-- =================================================================
PROCEDURE PROC_KPI1_REGISTROS IS
...
C_PROC_NAME CONSTANT VARCHAR2(30) := 'PROC_KPI1_REGISTROS';
...
END PROC_KPI1_REGISTROS;
-- =================================================================
-- PROCEDURE 5
-- =================================================================
PROCEDURE PROC_KPI5_ENTREGAS IS
--==================Definicion de variables==================--
C_PROC_NAME CONSTANT VARCHAR2(30) := 'PROC_KPI5_ENTREGAS';
RESULTADOS_KPI5 T_KPI5_ARRAY;
--==================Inicio de los procesos==================--
BEGIN
-- Limpieza de tabla de destino
DELETE FROM KPI_5;

WITH TASAS_CALCULADAS AS (
SELECT
...
GROUP BY TRUNC(FECHA_ENT_FIN, 'MM')
)
-- Recolección de resultados
SELECT
TO_CHAR(TC.MES, 'Month YYYY', 'NLS_DATE_LANGUAGE=SPANISH') AS PERIODO,
CASE WHEN TC.TOTAL_ENTREGAS_PROCESADAS > 0 THEN ROUND((TC.ENTREGAS_EXITOSAS / TC.TOTAL_ENTREGAS_PROCESADAS) * 100, 2) ELSE 0 END AS TASA_EXITOSAS,
-- Cálculo de la variación con LAG
CASE WHEN LAG(CASE WHEN TC.TOTAL_ENTREGAS_PROCESADAS > 0 THEN ROUND((TC.ENTREGAS_EXITOSAS / TC.TOTAL_ENTREGAS_PROCESADAS) * 100, 2) ELSE 0 END, 1, NULL) OVER (ORDER BY TC.MES) IS NOT NULL
    THEN ROUND(CASE WHEN TC.TOTAL_ENTREGAS_PROCESADAS > 0 THEN ROUND((TC.ENTREGAS_EXITOSAS / TC.TOTAL_ENTREGAS_PROCESADAS) * 100, 2) ELSE 0 END - LAG(CASE WHEN TC.TOTAL_ENTREGAS_PROCESADAS > 0 THEN ROUND((TC.ENTREGAS_EXITOSAS / TC.TOTAL_ENTREGAS_PROCESADAS) * 100, 2) ELSE 0 END, 1) OVER (ORDER BY TC.MES), 2) 
    ELSE NULL END AS VARIACION_TASA,
SYSDATE AS FECHA_ACTUALIZACION
BULK COLLECT INTO RESULTADOS_KPI5
FROM TASAS_CALCULADAS TC
ORDER BY TC.MES;

IF RESULTADOS_KPI5.COUNT > 0 THEN
FORALL i IN RESULTADOS_KPI5.FIRST .. RESULTADOS_KPI5.LAST
    INSERT INTO KPI_5 (PERIODO, TASA_EXITOSAS, VARIACION_TASA, FECHA_ACTUALIZACION)
    VALUES (RESULTADOS_KPI5(i).PERIODO, RESULTADOS_KPI5(i).TASA_EXITOSAS, RESULTADOS_KPI5(i).VARIACION_TASA, RESULTADOS_KPI5(i).FECHA_ACTUALIZACION);
END IF;

EXCEPTION WHEN OTHERS THEN RAISE;
END PROC_KPI5_ENTREGAS;


-- =================================================================
-- PROCEDURE 6
-- =================================================================
PROCEDURE PROC_KPI6_RECURRENCIA IS
--==================Definicion de variables==================--
C_PROC_NAME CONSTANT VARCHAR2(30) := 'PROC_KPI6_RECURRENCIA';
RESULTADOS_KPI6 T_KPI6_ARRAY;
--==================Inicio de los procesos==================--
BEGIN
DELETE FROM KPI_6;

WITH FRECUENCIA_ANUAL AS (
    SELECT TO_CHAR(FECHA_COM, 'YYYY') AS ANIO, CLIENTE_RUN_CLI, COUNT(COD_COM) AS COMPRAS_TOTALES
    FROM COMPRA GROUP BY TO_CHAR(FECHA_COM, 'YYYY'), CLIENTE_RUN_CLI
),
TASAS_ANUALES AS (
    SELECT
        FA.ANIO,
    ...
    FROM FRECUENCIA_ANUAL FA GROUP BY FA.ANIO ORDER BY FA.ANIO
)
SELECT
    TA.ANIO AS ANIO,
...
FROM TASAS_ANUALES TA;

IF RESULTADOS_KPI6.COUNT > 0 THEN
    FORALL i IN RESULTADOS_KPI6.FIRST .. RESULTADOS_KPI6.LAST
        INSERT INTO KPI_6 (ANIO, TOTAL_CLIENTES_UNQ, TASA_RECURRENCIA, VARIACION_TASA, FECHA_ACTUALIZACION)
        VALUES (RESULTADOS_KPI6(i).ANIO, RESULTADOS_KPI6(i).TOTAL_CLIENTES_UNQ, RESULTADOS_KPI6(i).TASA_RECURRENCIA, RESULTADOS_KPI6(i).VARIACION_TASA, RESULTADOS_KPI6(i).FECHA_ACTUALIZACION);
END IF;
